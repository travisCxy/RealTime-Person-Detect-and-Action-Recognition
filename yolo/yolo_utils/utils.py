...
def draw_boxes(orig_img, detect_size, detections, classes):
	img = orig_img
	color_list = [[random.randint(0, 255), random.randint(0, 255), random.randint(0, 255)] for _ in range(len(classes))]
	pad_x = max(img.shape[0] - img.shape[1], 0) * (detect_size / max(img.shape))
	pad_y = max(img.shape[1] - img.shape[0], 0) * (detect_size / max(img.shape))
	unpad_h = detect_size - pad_y
	unpad_w = detect_size - pad_x 
